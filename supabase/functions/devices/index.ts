...
async function getTask(supabaseClient: SupabaseClient, id: string) {
  const { data: task, error } = await supabaseClient.from('devices').select('*').eq('id', id)
...
async function getAllTasks(supabaseClient: SupabaseClient) {
  const { data: tasks, error } = await supabaseClient.from('devices').select('*')
  if (error) throw error
...
async function updateTask(supabaseClient: SupabaseClient, id: string, task: Task) {
  const { error } = await supabaseClient.from('devices').update(task).eq('id', id)
  if (error) throw error
...
async function createTask(supabaseClient: SupabaseClient, task: Task) {
  const { error } = await supabaseClient.from('devices').insert(task)
  if (error) throw error
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const taskPattern = new URLPattern({ pathname: '/devices/:id' })
